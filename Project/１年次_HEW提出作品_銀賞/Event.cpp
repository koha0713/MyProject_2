...
// チュートリアルイベント
void Event1_Draw(Player* p, Pic* pic, Map* m,Enemy* e) {
	...
	if (p->text[0] == 1) {
		...
		// テキストの描写
        SetPalette(pic->e1_t1[p->text[1]-1]);
        DrawBmp(160, 352, pic->e1_t1[p->text[1]-1], true);
        // キャラアイコン
        SetPalette(pic->tatie_king);
        DrawBmp(10, 362, pic->tatie_king, true);
...
void Event1_Update(Player* p, Key* k, Map* m, Enemy* e) {
    if (p->bgm_t == 1) {
        MciPlaySound(p->bgm[1], true);
        p->bgm_t = 0;
    }
	if (k->key_enter_T == 1) {
		p->text[1]++;
...
void Event2_Draw(Player* p, Pic* pic, Map* m, Enemy* e) {
    // 背景描写
    ...
    if (e->flug_enemy == 1) {
        SetPalette(pic->e_0);
        DrawBmp(e->enemy[e->now_enemy][2], e->enemy[e->now_enemy][4], pic->e_0, true);
        if (e->flug_damage == 1) {
            if (p->ani_cnt2 < 2) {
                switch (p->B_x) {
                case 1:
...
void Event2_Update(Player* p, Key* k, Map* m, Enemy* e) {
    if (p->bgm_t == 1) {
        MciStopSound(p->bgm[1]);
    ...
    if (e->flug_enemy == 1) {
        ...
        // 落下時障害物判定(下)
        if (p->flug_Jump == 0) {
            switch (m->stage_0[(p->B_Bottom / 32) + 1][p->Center[0] / 32]) {
            case 3:
                switch (p->FallCount) {
                case 0:
                case 1:
...
void Event3_Draw(Player* p, Pic* pic, Map* m) {
    // マップチップ描写
    for (int x = 0; x < 20; x++) {
        for (int y = 0; y < 16; y++) {
            SetPalette(pic->k_4);
    ...
    // テキスト描写
    // テキストフラグの確認
    if (p->text[0] == 1 && p->text[1] < 6) {
        // テキストボックスの描写
        SetPalette(pic->textbox);
        DrawBmp(0, 352, pic->textbox, false);
        // テキストの描写

        SetPalette(pic->e1_t2[p->text[1]]);
        DrawBmp(160, 352, pic->e1_t2[p->text[1]], true);
        switch (p->text[1]) {
        case 0:
            SetPalette(pic->tatie_king);
            DrawBmp(10, 362, pic->tatie_king, true);
...
void Event3_Update(Player* p, Key* k, Map* m) {
    if (p->bgm_t == 1) {
        MciStopSound(p->bgm[2]);
        MciPlaySound(p->bgm[1], true);
        p->bgm_t = 0;
...
// 中ボスイベント
void Event4_Draw(Player* p, Pic* pic, Map* m) {
    // マップチップ描写
    for (int x = 0; x < 20; x++) {
        for (int y = 0; y < 16; y++) {
            SetPalette(pic->k_4);
...
void Event4_Update(Player* p, Key* k, Map* m) {
    if (k->key_enter_T == 1) {
        p->text[1]++;
    }
    if (p->text[1] == 7) {
